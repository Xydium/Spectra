...
namespace spec
{
    ...
    class Spectra
    {
        ...
        private void Process()
        {
            ...
            while (running)
            {
                lock(Lock)
                {
                    ...
                    //Array.ForEach(stream, b => Console.Write("{0} ", b));
                    //Console.WriteLine();
        ...
        private void InitVars()
        {
            ...
            for(int i = 0; i < 20; i++)
            {
                v.Add("SCRIPT_ARG_" + i, new Variant(0));
            }
        }
        ...
        public void Run(string[] args)
        {
            if(File.Exists(args[0]))
            {
                ...
                lock (Lock) {
                    ...
                    {
                        v["SCRIPT_ARG_" + (i - 1)] = Variant.Parse(args[i]);
                        Console.WriteLine("SCRIPT_ARG_{0} = {1}", i - 1, args[i]);
                    }
                }
    ...
    class LinearGen : Processor
    {
        ...
        public LinearGen(Variant[] args) : base(args) {
            if (args == null) return;
            this.minValue = args[0].i;
            this.maxValue = args[1].i;
        }

        override public void Process(Spectra s, Dictionary<String, Variant> v, byte[] b, byte[] bb, byte[] t)
        {
            float dvdx = (maxValue - minValue) / (float) (b.Length - 1);

            for(int i = 0; i < b.Length; i++)
            {
                b[i] = (byte) (minValue + dvdx * i);
            }
        }
    ...
    class ConstantSineGen : Processor
    {
        ...
        public ConstantSineGen(Variant[] args) : base(args)
        {
            ...
            this.minimum = args[1].i;
            this.maximum = args[2].i;
        }
        ...
        override public void Process(Spectra s, Dictionary<String, Variant> v, byte[] b, byte[] bb, byte[] t)
        {
            ...
            for (int i = 0; i < b.Length; i++)
            {
                b[i] = (byte) (halfdiff * Math.Sin(6.28 / period * v["SPECTRA_TIME"].f) + midpoint);
            }
        }
    ...
    class CopyProc : Processor
    {
        ...
        public CopyProc(Variant[] args) : base(args)
        {
            if (args == null) return;
    ...
    class RangeProc : Processor
    {
        ...
        {
            if (args == null) return;
            this.minimum = args[0].i;
            this.maximum = args[1].i;
        }
        ...
        override public void Process(Spectra s, Dictionary<String, Variant> v, byte[] b, byte[] bb, byte[] t)
        {
            for (int i = 0; i < b.Length; i++)
            {
                b[i] = (byte) (minimum + (maximum - minimum) * (b[i] / 255f));
            }
        }
 ...
 *  
 *  Create with defaults function to fill in values for script variables
 */
